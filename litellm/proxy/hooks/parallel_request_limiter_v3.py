    ...
    def _get_total_tokens_from_usage(self, usage: Any | None, rate_limit_type: Literal["output", "input", "total"]) -> int:
        ...
        if usage:
            if isinstance(usage, Usage):
                ...
                # Fallback for dict-based usage (might come from some edge cases)
                if rate_limit_type == "output":
                    total_tokens = usage.get("completion_tokens", 0) or usage.get("output_tokens", 0)
                elif rate_limit_type == "input":
                    total_tokens = usage.get("prompt_tokens", 0) or usage.get("input_tokens", 0)
                elif rate_limit_type == "total":
